...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        q_values: Sequence[np.ndarray] = [self._calculate_q_vals(rews) for rews in rewards]
        ...
        batch_size = len(obs) * obs.size

        obs = np.concatenate([np.array(ob) for ob in obs], axis=0)
        actions = np.concatenate([np.array(ac) for ac in actions], axis=0)
        rewards = np.concatenate([np.array(re) for re in rewards], axis=0)
        terminals = np.concatenate([np.array(te) for te in terminals], axis=0)
        q_values = np.concatenate([np.array(qv) for qv in q_values], axis=0)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            with torch.no_grad():
                values = self.critic(obs).cpu().detach().numpy()
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i]:
                        ...
                        advantages[i] = advantages[i] + self.gae_lambda * self.gamma * advantages[i+1]
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        return [sum(self.gamma ** i * reward for i, reward in enumerate(rewards))] * len(rewards)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        discounted = [self.gamma ** i * reward for i, reward in enumerate(rewards)]
        return [sum(discounted[i:]) for i in range(len(rewards))]
